...
se_table_a=se[c(4,7,8)][!duplicated(se[c(4,7,8)]),] #103156
colnames(se_table_a)[c(2,3)]=c('umls','se')
se_table_b=se[c(4,3,5)][!duplicated(se[c(4,3,5)]),] #99423
colnames(se_table_b)[c(2,3)]=c('umls','se')
se_table_1=rbind(se_table_a,se_table_b)
se_table_1$se=str_to_lower(se_table_1$se)
se_table_1=se_table_1[!duplicated(se_table_1),]#147217
dim(se_table_1)
...
se_table_matchmiremoved_1=remove.matchse_mi_1[c(1:3)][!duplicated(remove.matchse_mi_1[c(1:3)]),]
...
se_icd_all_1=se_icd_all_1[!duplicated(se_icd_all_1),]
...
matchingphecode_mi_se= se_icd_all_1 %>% select(se,ICD10_SE, PHECODE_SE, mi, ICD10_MI, PHECODE_MI) %>% filter(PHECODE_SE==PHECODE_MI) %>% distinct()
...
finalse_df= se_icd_all_1 %>% select(drugname,PHECODE_SE, se_adr) %>% distinct()
